...
from .spectral import spectral_layout, tswspectral_layout
from .layouts import (
    optimize_layout_euclidean,
    optimize_layout_generic,
    optimize_layout_inverse,
    optimize_layout_euclidean_with_ghost,
)
...
def simplicial_set_embedding_with_ghosts(
    data,
    graph,
    n_components,
    initial_alpha,
    ...
    embedding, ghost_embedding, ghost_targets = optimize_layout_euclidean_with_ghost(
        embedding,
        embedding,
        head,
        tail,
    ...
    return embedding, aux_data, ghost_embedding, ghost_targets
...
class UMAP(BaseEstimator):
    ...
    def __init__(
        ...
        self.ghost_embedding = None
        self.ghost_targets = None
    ...
    def fit(self, X, y=None, force_all_finite=True, ghost_mode=False):
        ...
        if self.transform_mode == "embedding":
            ...
            if ghost_mode:
                self.embedding_, aux_data, self.ghost_embedding, self.ghost_targets = (
                    self._fit_embed_data_with_ghost(
    ...
    def _fit_embed_data_with_ghost(self, X, n_epochs, init, random_state):
        """A method wrapper for simplicial_set_embedding that can be
        replaced by subclasses.
        """
        print("Embedding with ghosts")
        return simplicial_set_embedding_with_ghosts(
            X,
            self.graph_,
            self.n_components,
            self._initial_alpha,
    ...
    def fit_transform(self, X, y=None, force_all_finite=True, ghost_mode=False):
        ...
        """
        print("UMAP")
        self.fit(X, y, force_all_finite, ghost_mode)
        if self.transform_mode == "embedding":
            if self.output_dens:
                return self.embedding_, self.rad_orig_, self.rad_emb_
            elif ghost_mode:
                return self.embedding_, self.ghost_embedding, self.ghost_targets
            else:
