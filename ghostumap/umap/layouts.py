...
def _optimize_layout_euclidean_single_epoch(
    ...
    for i in numba.prange(epochs_per_sample.shape[0]):
        if epoch_of_next_sample[i] <= n:
            ...
            current = head_embedding[j]  # position of the 1st node
            other = tail_embedding[k]  # position of the 2nd node
...
def _optimize_layout_euclidean_with_ghost_single_epoch(
    head_embedding,
    tail_embedding,
    head,
    tail,
    ...
    """
    ghost_embedding: array of shape (n_ghosts, n_ghosts_per_target, n_components)
        Embeddings of ghosts
    ghost_target_idx: dict
        Index of ghost target
    ghost_existence: array of shape (n_vertices,)
        Whether ghost exists
    ...
    for i in numba.prange(epochs_per_sample.shape[0]):
        if epoch_of_next_sample[i] <= n:
            ...
            for g in range(num_ghosts_per_target):
                current = ghost_embedding[ghost_target_idx[j]][
                    g
                ]  # position of the 1st node
                other = tail_embedding[k]  # position of the 2nd node
...
def optimize_layout_euclidean_with_ghost(
    head_embedding,
    tail_embedding,
    head,
    tail,
    ...
    optimize_fn = numba.njit(
        _optimize_layout_euclidean_with_ghost_single_epoch,
        fastmath=True,
        parallel=parallel,
    )
    ...
    num_ghosts_per_target = 10
    ghost_embedding = np.array(
        [
            np.tile(head_embedding[i], (num_ghosts_per_target, 1))
            for i in range(n_vertices)
        ]
    )
    has_ghost = np.array([True for _ in range(n_vertices)])
    ghost_targets = np.array([i for i in range(n_vertices)])
    ghost_target_idx = np.array([i for i in range(n_vertices)])

    filtering = [500, 5000, 20000]

    for n in tqdm(range(n_epochs), **tqdm_kwds):
        if n != 0 and n % 50 == 0:
            center_of_ghost = np.mean(ghost_embedding, axis=1)
            var_of_ghost = np.array(
                [
                    np.linalg.norm(a - center_of_ghost[i])
                    for i, a in enumerate(ghost_embedding)
                ]
            )

            ghost_var_rank = np.argsort(var_of_ghost)[::-1]
            normal_ghosts = ghost_var_rank[filtering[-1] :]
            abnormal_ghosts = np.sort(ghost_var_rank[: filtering[-1]])

            print(abnormal_ghosts[:20])
            filtering.pop()

            has_ghost[ghost_targets[normal_ghosts]] = False
            ghost_targets = np.where(has_ghost)[0]
            ghost_target_idx[ghost_targets] = np.arange(len(ghost_targets))
            ghost_embedding = ghost_embedding[abnormal_ghosts]
            print(ghost_embedding.shape, ghost_targets.shape)
            if n % 100 == 50:
                ghost_embedding = np.tile(ghost_embedding, (1, 2, 1))
                num_ghosts_per_target *= 2
            print(ghost_embedding.shape)
            np.save(f"./result/ghost_embedding_epoch_{n}.npy", ghost_embedding)
            np.save(f"./result/ghost_targets_epoch_{n}.npy", ghost_targets)
    ...
    # np.save("./result/ghost_embedding.npy", ghost_embedding)
    # np.save("./result/ghost_targets.npy", ghost_targets)

    return (
        head_embedding if epochs_list is None else embedding_list,
        ghost_embedding,
        ghost_targets,
    )
